...
if train_file is not None:
    try:
        ...
        with c4:
            hidden_layer_size = st.selectbox("NN hidden neurons", [16, 32, 64, 128], index=1)
        ...
        if st.button("Train and Compare Models"):
            ...
            # ---------------------------------
            # Scaling for Linear + Neural Network
            # ---------------------------------
            ...
            nn_metrics = evaluate_model(y_train, y_train_pred_nn, y_test, y_test_pred_nn, p)

            # -----------------------------
            # Comparison table
            # -----------------------------
            comparison_df = pd.DataFrame({
                "Model": ["Linear Regression", "Random Forest", "Hybrid", "Neural Network"],
                "Train R²": [
                    lin_metrics["Train R²"],
                    rf_metrics["Train R²"],
                ...
                "Interpretation": [
                    ...
                    interpret_model(nn_metrics["Train R²"], nn_metrics["Test R²"])
                ]
            })
    ...
    if pred_file is not None:
        try:
            ...
            if missing_raw_cols:
                ...
                pred_scaled = scaler.transform(pred_encoded)
                ...
                if best_model_name == "Linear Regression":
                    ...
                    linear_preds = best_model["linear_model"].predict(pred_scaled)
